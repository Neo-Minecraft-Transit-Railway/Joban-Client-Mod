        ...
        if(world != null && !world.isClient() && JCMServerStats.getGameTick() % 5 == 0) {
            ...
            MinecraftServerHelper.iteratePlayers(ServerWorld.cast(getWorld2()), (player) -> {
                if(box.contains(player.getPos())) {
                    ...
                    if(!alreadyOpened) {
                        world.playSound((Entity) null, pos, SoundHelper.createSoundEvent(new Identifier("minecraft:block.iron_door.open")), SoundCategory.BLOCKS, 1, 1);
            ...
            if(!haveNearbyPlayer.get() && IBlock.getStatePropertySafe(state, new Property<>(DoorBlockAbstractMapping.getOpenMapped().data))) {
                world.playSound((Entity) null, getPos2(), SoundHelper.createSoundEvent(new Identifier("minecraft:block.iron_door.close")), SoundCategory.BLOCKS, 1, 1);
