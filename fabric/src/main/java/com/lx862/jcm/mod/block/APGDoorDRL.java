...
public class APGDoorDRL extends BlockPSDAPGDoorBase implements BlockWithEntity {
    ...
    public VoxelShape getCollisionShape2(BlockState state, BlockView blockView, BlockPos pos, ShapeContext shapeContext) {
        // 1.21.11 caches collision shapes at register time with an empty BlockGetter.
        // Mapping getBlockEntity wraps a null vanilla BE instead of returning null.
        final BlockEntity blockEntity = blockView.getBlockEntity(pos);
        if (blockEntity == null || !(blockEntity.data instanceof BlockEntityBase) || blockEntity.getWorld() == null || !blockEntity.getWorld().isClient()) {
